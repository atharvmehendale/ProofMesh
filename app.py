...
_SECRET_KEYS = ["FEATHERLESS_API_KEY"]
...
def run_pipeline(extracted: list[dict], secrets: dict, source_filename: str = "pasted_text") -> ProofAuditResult:
    ...
    accidentally double an extraction call. Kept separate from
...
def main() -> None:
    ...
    secrets = get_secrets()
    st.caption(f"Key found: {bool(secrets.get('FEATHERLESS_API_KEY'))}")
    if not secrets.get("FEATHERLESS_API_KEY"):
        st.error(
            "No API key configured. Set FEATHERLESS_API_KEY in Streamlit "
